import { traceSummary, traceSummaries, mkDurationStr, totalDuration, detailedTraceSummary } from './trace';
...
describe('detailedTraceSummary', () => {
  ...
  it('should derive summary info', () => {
    const { traceId, durationStr, depth, serviceNameAndSpanCounts } = detailedTraceSummary(cleanedHttpTrace);
  ...
  it('should position incomplete spans at the correct offset', () => {
    ...
    expect(spans.map(s => s.left)).toEqual([0, 8.108108108108109, 16.216216216216218, 64.86486486486487]);
  });
  ...
  it('should derive summary info even when headless', () => {
    ...
    const { traceId, durationStr, depth, serviceNameAndSpanCounts } = detailedTraceSummary(headless);
