...
class DetailedTraceSummary extends React.Component {
  ...
  renderHeader() {
    ...
    const { durationStr, serviceNameAndSpanCounts, spans, depth } = traceSummary;

    return (
      <div className="detailed-trace-summary__header">
        <div className="detailed-trace-summary__trace-id">{traceId}</div>
        <div className="detailed-trace-summary__trace-data-list">
          {[
            ...
            <div key={elem.label} className="detailed-trace-summary__data-wrapper">
              <div className="detailed-trace-summary__data-label">{elem.label}</div>
              <div className="detailed-trace-summary__data-value">{elem.value}</div>
            </div>
          ))}
        </div>
      </div>
    ...
    return (
      <div>
        <div className="detailed-trace-summary">
          {!traceSummary || traceSummary.traceId !== traceId ? null : (
            <div>
