...
class TimelineSpan extends React.Component {
  ...
  renderServiceNameColumn() {
    ...
    return (
      <div className="timeline-span__service-name-column">
        ...
        />
        <div className="timeline-span__service-name-wrapper" style={{ left: `${(span.depth + 1) * 14}px` }}>
          <div className="timeline-span__service-name">{this.renderServiceName()}</div>
        </div>
      </div>
    );
  }
  ...
  renderTimeMarkers() {
    ...
    for (let i = 1; i < numTimeMarkers - 1; i += 1) {
      ...
      timeMarkers.push(
        <span key={portion} className="timeline-span__time-marker" style={{ left: `${portion * 100}%` }} />,
      );
    }
    return timeMarkers;
  }
  ...
  renderSpanDuration(left, width) {
    ...
    return (
      <span className="timeline-span__duration timeline-span__duration--left" style={{ left: `${left}%` }}>
        {span.durationStr}
      </span>
    );
  }
        ...
        {measures.map(({ width, left, isBelongToChild }) => {
          return (
            <>
              ...
              {!isBelongToChild && showDuration && this.renderSpanDuration(left, width)}
            </>
          );
        })}
      </div>
  ...
  render() {
    ...
    return (
      <div>
        <div role="presentation" className={timelineSpanClass} onClick={this.handleDataOpenToggle}>
          <div
          ...
          {showSpanDetailTitle && (
            <div className="timeline-span__span-name-column-wrapper" style={{ width: `${spanNameColumnWidth * 100}%` }}>
              <div className="timeline-span__span-name-column">{span.spanName}</div>
            </div>
          )}
          <div
            className="timeline-span__bar-wrapper"
            style={{
              width: `${(1 - (serviceNameColumnWidth + showSpanDetailTitle ? spanNameColumnWidth : 0)) * 100}%`,
            }}
          >
