...
class TimelineHeader extends React.Component {
  ...
  handleMouseMove(event) {
    const { serviceNameColumnWidth, onServiceNameColumnWidthChange, onSpanNameColumnWidthChange } = this.props;
    const { resizingColumnName } = this.state;

    if (resizingColumnName === serviceNameColumn) {
      onServiceNameColumnWidthChange(Math.max(this.getPosition(event.clientX), minServiceNameColumnWidth));
    } else if (resizingColumnName === spanNameColumn) {
      onSpanNameColumnWidthChange(
        Math.max(this.getPosition(event.clientX) - serviceNameColumnWidth, minSpanNameColumnWidth),
      );
    }
  }
  ...
  renderResizableColumn(columnName) {
    ...
    return (
      <div className="timeline-header__resizable-column" style={{ width: `${columnWidth * 100}%` }}>
  ...
  renderTimeMarkers() {
    const { startTs, endTs, numTimeMarkers, serviceNameColumnWidth, spanNameColumnWidth, showSpanDetailTitle } =
      this.props;
