...
const renderTickLines = () => {
  ...
  for (let i = 1; i < NUM_TICKS - 1; i += 1) {
    ...
    timeMarkers.push(<line key={portion} x1={`${portion}%`} x2={`${portion}%`} y1="0" y2={GRAPH_HEIGHT} />);
  }
  ...
  renderTicks() {
    ...
    for (let i = 0; i < NUM_TICKS; i += 1) {
      ...
      timeMarkers.push(
        ...
        >
          <span className={`mini-trace-viewer__time-marker-label ${portionClassName}`}>{label}</span>
        </div>,
      );
    }
    return <div>{timeMarkers}</div>;
  }
  ...
  render() {
    ...
    return (
      <div className="mini-trace-viewer">
        <div className="mini-trace-viewer__time-markers-wrapper">{this.renderTicks()}</div>
        <div
          ...
          <svg version="1.1" width="100%" height={GRAPH_HEIGHT} xmlns="http://www.w3.org/2000/svg">
            ...
            {isDragging ? (
              <g stroke="#999" strokeWidth="1">
                <line x1={`${dragStartX * 100}%`} x2={`${dragStartX * 100}%`} y1={0} y2={GRAPH_HEIGHT} />
                <line
                  x1={`${dragStartX * 100}%`}
                  x2={`${dragCurrentX * 100}%`}
                  y1={GRAPH_HEIGHT / 2}
                  y2={GRAPH_HEIGHT / 2}
                />
                <line x1={`${dragCurrentX * 100}%`} x2={`${dragCurrentX * 100}%`} y1={0} y2={GRAPH_HEIGHT} />
              </g>
            ) : null}
            {startTs ? (
              <rect
